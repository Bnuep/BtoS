...
function CObj(n, s, x, tx, ti){
	...
	this.anim = function () {
		with(this){
			...
			l   = (x1*si)+6*(n+1);
			w   = zo*si;
			is.style.left   = Math.round(l+30)+'px';
			is.style.top    = Math.round((H-w*rImg)*.25)+'px';
			is.style.width  = Math.round(w*.4)+'px';
			is.style.height = Math.round(w*rImg*.4)+'px';
			if(sel == n){
				if(sel<nI*.5) {
					tit.style.left = txt.style.left = Math.round(l+w-120)+'px';
				} else {
					tit.style.left = txt.style.left = Math.round(l-(nx*.25)+150)+'px';
				}
				txt.style.top = Math.round(-(w*rImg)*.1)+'px';
				tit.style.top = Math.round((w*rImg)*.1)+'px';
			}
		}
	}
}
...
function doResize(){
	tit.style.width = Math.round(nx*.1)+'px';
	txt.style.width = Math.round(nx*.1)+'px';
	tit.style.fontSize = (nx/80)+'px';
	txt.style.fontSize = (nx/70)+'px';
	with(box.style){
		width  = Math.round(W-30)+'px';
