...
function dataAttr( elem, key, data ) {
	...
	if ( data === undefined && elem.nodeType === 1 ) {
		...
		if ( typeof data === "string" ) {//如果数据类型为字符串
			try {
				data = data === "true" ? true ://如果数据等于【true】将true赋值给变量【data】否则执行下一步
					data === "false" ? false :
					data === "null" ? null :
					...
					rbrace.test( data ) ? jQuery.parseJSON( data ) :
						data;//这三元运算表达式真复杂！！！
					//test函数：检索为字符串的数据是否匹配
					//parseJSON函数：接收一个JSON字符串，返回解析后的对象
			} catch( e ) {}
...
//创建鼠标进入/鼠标离开事件，使用鼠标在上面/鼠标离开和时间检查事件。
jQuery.each({
...
function createTweens( animation, props ) {
	jQuery.each( props, function( prop, value ) {
		var collection = ( tweeners[ prop ] || [] ).concat( tweeners[ "*" ] ),//concat() 函数用于连接两个或者多个数组
			index = 0,
